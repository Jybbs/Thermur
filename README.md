...
-   **Scale-Free Correlation and Information Propagation**: A murmuration exists in a "critical" state, much like a physical system at a phase transition [^13]. This critical state is maintained through individual heterogeneity—approximately 30% of birds maintain heightened vigilance at any time [^32], creating natural perturbations that propagate through alignment interactions. The susceptibility χ = N·Var[Φ], where Φ is the polarization order parameter, quantifies the flock's responsiveness to these perturbations [^7]. These alert states persist according to Markovian dynamics with asymmetric transition rates [^33], creating realistic bout durations rather than random switching each timestep. This allows information, such as a change in direction in response to a threat, to propagate across the entire flock almost instantaneously, regardless of the flock's size [^13]. For Thermur, this property is paramount. By implementing individual-based alert states where agents have heterogeneous noise levels, the swarm naturally maintains the variance required for high susceptibility. When treating fire as the "predator," individual agents stochastically enter alert states, creating seeds of perturbation that ripple through the flock, reflecting large-scale wind shifts and turbulent gusts without a central controller.
